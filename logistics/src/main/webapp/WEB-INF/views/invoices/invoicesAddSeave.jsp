                        ...
                        <div class="invoices-add-1">
                            ...
                            <div class="invoices-add-1-table">
                                <table class="table">
                                    ...
                                    <tbody>
                                    <c:forEach items="${orderMap}" var="obj"  >
                                        <c:forEach items="${obj}" var="order">
                                        <tr>
                                            <td>
                                                 <input name="orderInputCheckbox" type="checkbox" value="${order.id }" >
                                            </td>
                                            <td>${order.createTime}</td>
                                            <td>
                                                <c:if test="${order.releaseSource.transportType == 1}">
                                                    陆运
                                                </c:if>
                                                <c:if test="${order.releaseSource.transportType == 2}">
                                                    海运
                                                </c:if>
                                                <c:if test="${order.releaseSource.transportType == 3}">
                                                    联运
                                                </c:if>
                                                <c:if test="${order.releaseSource.transportType == 4}">
                                                    航运
                                                </c:if>
                                            </td>
                                            <td>${order.startAddress}</td>
                                            <td>${order.arriveAddress}</td>
                                            <td>${order.releaseSource.userFront.userName}</td>
                            ...
                            <div class="invoices-add-1-button">
                                <a href="javascript:orderSubmitInvoice(0)" class="btn btn-primary">下一步</a>
                                <a href="javascript:InvoiceAddSaveCancel(0)" class="btn btn-default">取消</a>
                            </div>
                        </div>
                    </div>
                </div>
